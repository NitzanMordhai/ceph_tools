...
if __name__ == "__main__":
    ...
    parser.add_argument('--error-message', help='only find that error message and send the report about that error message by dates', default=None)
    args = parser.parse_args()

    if args.error_message:
        subprocess.run(
            ['python', 'scan_scrapy_error_message.py', '--error_message', args.error_message, '--db_name', args.db_name]
        )
        exit(0)
