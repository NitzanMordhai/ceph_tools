...
def scan_scrapy_directories(
    ...
    **kwargs,
    ...
    pattern = (
        rf"^{users_pat}-"                # user or wildcard
        rf"{date_re}-"
        rf"{suite_pat}{branch_pat}-"     # suite plus any branch/extra bits
        rf"{re.escape(version)}"         # version literal
        rf"(?:-release)?-distro-"        # optional “-release”, then “-distro-”
        rf"{re.escape(flavor)}-smithi$"  # flavor + host
    )
